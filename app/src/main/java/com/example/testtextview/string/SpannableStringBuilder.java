...
import com.example.testtextview.textview.GraphicsOperations;
...
public class SpannableStringBuilder implements CharSequence, GetChars, Spannable, Editable,
    ...
    public SpannableStringBuilder(CharSequence text, int start, int end) {
        int srclen = end - start; // 获取实际的长度

        if (srclen < 0) throw new StringIndexOutOfBoundsException(); // 异常处理

        mText = ArrayUtils.newUnpaddedCharArray(GrowingArrayUtils.growSize(srclen)); // 创建mText数组
     ...
     * @param end 替换结束位置
     * @param tb 替换的字符串资源
     * @param tbstart 替换字符资源的开始位置
     * @param tbend 替换字符资源的结束位置
     * @return
     */
    ...
    private void setSpan(boolean send, Object what, int start, int end, int flags,
        ...
        mSpanFlags = GrowingArrayUtils.append1(mSpanFlags, mSpanCount, flags);
        mSpanOrder = GrowingArrayUtils.append1(mSpanOrder, mSpanCount, mSpanInsertCount);
...
//        }
    ...
    /**
     * 缺口长度，表示mText申请的内存长度减去 已经使用了的长度，还剩下多少没用的长度
     */
