...
//        android.text.Layout.Alignment align, TextDirectionHeuristic textDir,
//        float spacingmult, float spacingadd,
//        boolean includepad,
...
//            reflowed = new StaticLayout(null);
            reflowed = new StaticLayout(text, where, where + after,
                    getPaint(), getWidth(),
                    android.text.Layout.Alignment.ALIGN_NORMAL,
                    getSpacingMultiplier(), getSpacingAdd(),
            false,
            mEllipsizeAt, mEllipsizedWidth);
...
//            objects[0] = reflowed.getLineDirections(i);
            objects[0] = DIRS_ALL_LEFT_TO_RIGHT;
...
//        b.finish();
    ...
    private boolean contentMayProtrudeFromLineTopOrBottom(CharSequence text, int start, int end) {
        ...
        getTextBounds(paint, text, start, end, mTempRect);
    ...
    public void getTextBounds(Paint paint, CharSequence text, int start, int end, Rect bounds) {
        ...
        if (bounds == null || paint == null) {
            throw new NullPointerException("need bounds Rect");
        }
        char[] buf = TemporaryBuffer.obtain(end - start);
        TextUtils.getChars(text, start, end, buf, 0);
        paint.getTextBounds(buf, 0, end - start, bounds);
    ...
    private void addBlockAtOffset(int offset) {
        ...
        if (line > previousBlockEndLine) {
            mBlockEndLines = GrowingArrayUtils.append1(mBlockEndLines, mNumberOfBlocks, line);
            updateAlwaysNeedsToBeRedrawn(mNumberOfBlocks);
            mNumberOfBlocks++;
        }
    }
