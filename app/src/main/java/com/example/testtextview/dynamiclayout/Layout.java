...
//            paint.setHyphenEdit(0);
    ...
    public long getLineRangeForDraw(Canvas canvas) {
        ...
        return packRangeInLong(getLineForVertical(top), getLineForVertical(bottom));
    }
    ...
    public long getRunRange(int offset) {
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            int limit = start + (runs[i+1] & RUN_LENGTH_MASK);
            if (offset >= start && offset < limit) {
                return packRangeInLong(start, limit);
            }
        }
        // Should happen only if the offset is "out of bounds"
        return packRangeInLong(0, getLineEnd(line));
...
//        mPaint.setHyphenEdit(0);
    ...
    public void getCursorPath(int point, Path dest,
        ...
        int caps = TextKeyListener.getMetaState(editingBuffer, TextKeyListener.META_SHIFT_ON) |
                TextKeyListener.getMetaState(editingBuffer, /*TextKeyListener.META_SELECTING*/0x800);
